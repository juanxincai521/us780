...
static u32 unblank_retry_cnt = 0;
static int mdp_on(struct platform_device *pdev)
{
	...
	timeout_check = panel_next_on(pdev);
	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
	...
	unblank_retry_cnt++;
	if(timeout_check < 0 && unblank_retry_cnt < 4){
		pr_info("%s === mipi-dsi start fail & try again\n", __func__);
		msleep(5); mdp_off(pdev);
		msleep(5); mdp_on(pdev);
	}
	unblank_retry_cnt = 0;
	pr_debug("%s:-\n", __func__);
